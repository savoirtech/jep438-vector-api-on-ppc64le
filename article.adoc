...
The implementation of Java Vector API has direct support for x64 and AArch64 CPUs. As PPC64LE is neither of these when JIT produces code, it'll be non-optimized to the platform. When disable automatic SIMD, we can see a dramatic drop in performance, similar to when we force branch-less vector code.
